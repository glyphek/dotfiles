...
# nsxiv takes no file name arguments for the first image, just the number.  Copy
# this file somewhere into your $PATH and add this at the top of rifle.conf:
#
#   mime ^image, has nsxiv, X, flag f = path/to/this/script -- "$@"
#
...
#
# Second, we need to append a file list to nsxiv, which can only be done
...
#   feed the list to nsxiv. This is the fastest approach since we won't have to
