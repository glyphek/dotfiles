...
#    if [[ ! -z $NOTMUTED ]] ; then
#        VOL=$(amixer sget Master | sed 's/%//g' | awk -F"[][]" '/dB/ { print $2 }' )
#        if [ $VOL -ge 85 ] ; then
#            echo -e "${VOL}%"
#        elif [ $VOL -ge 50 ] ; then
#            echo -e "${VOL}%"
...
Volume(){
    ...
    echo "vol: $ANCHOR_VOLUME$msg"
}

while true; do
        echo "$(User) $(Workspace) [cpu: $(Cpu)] [mem: $(Memory)] [$(Volume)] [$(Battery)] $(Clock)"
	sleep 1s
done
